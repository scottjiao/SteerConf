...
def compute_conf_metrics(y_true, y_confs, min_max_range=[0, 1]):
    ...
    #assert all([x >= min_range and x <= max_range for x in y_confs]), y_confs
    if not all([x >= min_range and x <= max_range for x in y_confs]):
        #print("Warning: not all values in the range [0, 1]")
        print("min:", min(y_confs), "max:", max(y_confs))
        raise ValueError("Confidence values are not in the range [0, 1]")
