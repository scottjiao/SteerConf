...
def extract1(data):
    ...
    return result
def check_validity_dl_answer_conf(dl,extracted_answer, extracted_conf, task_type, error_log_file):
    ...
    try:
        ...
        with open(error_log_file, 'a') as f:
            f.write(f"Difficulty Level Invalid: {dl}\n")
        return None, None,None
    
    if extracted_answer is None:
        return None, extracted_conf,dl
    ...
    return extracted_answer, extracted_conf,dl
...
def extract_hint_response_answer_before_confidence(text, options: Dict[str, str], task_type, error_log_file):
    def default_postprocess_match(match) -> Tuple[str, str]:
        assert match is not None
        option_key, conf_scale = match.group(2), match.group(1)
        return option_key, conf_scale
    
    def postprocess_match_without_option(match) -> Tuple[str, str]:
        assert match is not None
        answer_option_value = match.group(2).strip()
    ...
    def postprocess_match_open_number(match) -> Tuple[str, str]:
        assert match is not None
        numerical_answer, conf_scale = match.group(2), match.group(1)
    ...
    # Define five different regular expression patterns
    patterns_multi_choice = [
        r"Confidence:\s*.*?(\d+)%*.*?[,.;]\s*[Aa]nswer:\s*.*?[\(\[]?([A-Z])[\)\]]?"
    ...
    # Define five different regular expression patterns
    patterns_open_number = [
        r"Confidence:\s*.*?(\d+)%*.*?[,.;]\s*[Aa]nswer:\s*.*?([0-9,.]+)"
    ]
    ...
    if not is_match:
        # If no match is found, print a message
        print(f"No `Confidence` match found for {text}")  
        with open(error_log_file, 'a') as f:
            f.write(f"No `Confidence` match found. extracted_answer: {answer}, extracted_conf: {conf}\n")
        answer = None
        conf = None
...
def extract_hint_response_difficulty_level(text, options: Dict[str, str], task_type, error_log_file):
    def default_postprocess_match(match) -> Tuple[str, str]:
        assert match is not None
        dl,option_key, conf_scale = match.group(1), match.group(2), match.group(3)
        return dl,option_key, conf_scale
    ...
    def postprocess_match_without_option(match) -> Tuple[str, str]:
        ...
        # assert answer_option_key is not None, (match.group(0), answer_option_value, options)
        if answer_option_key is None:
            print(match.group(1), answer_option_value, options)
            # it returns an answer that does not belong to any of the option values
            return dl,"Z", conf_scale
        
        return dl,answer_option_key, conf_scale
    
    def postprocess_match_open_number(match) -> Tuple[str, str]:
        assert match is not None
        dl,numerical_answer, conf_scale = match.group(1), match.group(2), match.group(3)
        numerical_answer = numerical_answer.replace(",", "") # 1,000 -> 1000
        
        return dl,numerical_answer, conf_scale
        
        
    # Define five different regular expression patterns
    patterns_multi_choice = [
        r"Difficulty Level, [aA]nswer and [cC]onfidence:\s*([0-9,.]+).*?,\s*[\(\[]?([A-Z])[\)\]]?[,.]?\s*(\d+)%*",
        r"Difficulty Level, [aA]nswer and [cC]onfidence:\s*([0-9,.]+).*?,\s*[\(\[]?([A-Z])[\)\]]?[,]?\s*[\(\[]?(\d+)%?[\(\[]?",
        r"Difficulty Level, [aA]nswer and [cC]onfidence:\s*([0-9,.]+).*?,\s*[\(\[]?([A-Z])[\)\]]?[,.]?\s*.*[\(\[]?(\d+)%?[\(\[]?",
    ]
    # sometimes the LLM will directly output the answer rather than the associated option key
    patterns_multi_choice_without_option = [
        r"Difficulty Level, [aA]nswer and [cC]onfidence:\s*([0-9,.]+).*?,\s*(.*?)\s*,\s*(\d+)%*"
    ]
    ...
    # Note: .* can match any character (except for a newline character) zero or more times
    patterns_multi_choice_werid = [
        r"Difficulty Level, ([0-9,.]+).*?,Answer:\s*[\(\[]?([A-Z])[\)\]]?[,.]?\s+Confidence level: (\d+%)",
        r"Difficulty Level, ([0-9,.]+).*?.*\s+Answer:\s*[\(\[]?([A-Z])[\)\]]?[,.]?.*\s+Confidence(?: level)?: (\d+%)",
        r"Difficulty Level:\s* ([0-9,.]+)[,.]?\s+Answer:\s*[\(\[]?([A-Z])[\)\]]?[,.]?\s+Confidence level:\s*(\d+%)"
    ]
    ...
    # Define five different regular expression patterns
    patterns_open_number = [
        r"Difficulty Level, [aA]nswer and [cC]onfidence:\s*.*?(\d+).*?[,\.;]\s*([0-9,.-]+).*?[,\.;]\s*(\d+)%*",
    ]
    ...
    for pattern, match_processor in patterns_and_postprocess:
        ...
        if not match:
            continue
        dl,answer, conf = match_processor(match)
        answer, conf , dl = check_validity_dl_answer_conf(dl,answer, conf, task_type, error_log_file)
        if answer is not None and conf is not None:
            is_match = True
            break     
