...
from utils.extract_result_lib import extract_hint_response_vanilla, extract_hint_response_self_evaluate, extract_hint_response_top_k, extract_hint_response_multistep,extract_hint_response_difficulty_level,extract_hint_response_answer_before_confidence
...
for question, answer in result_data.items():
    ...
    for hint_key, hint_value in answer.items():
        ...
        if args.prompt_type in ["vanilla", "cot", 
                                "confidence_after_answer_important_decisions_confident","confidence_after_answer_important_decisions_very_confident","confidence_after_answer_important_decisions_cautious","confidence_after_answer_important_decisions_very_cautious"]:
        ...
        if args.sampling_type == "self_random":
            ...
            hint_dls[hint_key]=extracted_dls
            ...
            elif hint_key in ['hint'+str(i) for i in range(1,30)]:
                hint_answers[hint_key] = extracted_answer
                hint_confs[hint_key] = extracted_conf
                hint_multi_step_confs[hint_key] = step_confs
                hint_dls[hint_key]=extracted_dls 
            else:
                #print(f"hint_key: {hint_key}, hint_key==hint0:{hint_key=='hint0'}")
                raise ValueError(f"The hint key {hint_key} is not in the list: ['hint0', 'hint1', 'hint2', 'hint3', 'hint4', ....]")

    if args.prompt_type in ["vanilla", "cot",   "confidence_after_answer_important_decisions_confident","confidence_after_answer_important_decisions_very_confident","confidence_after_answer_important_decisions_cautious","confidence_after_answer_important_decisions_very_cautious"]:
